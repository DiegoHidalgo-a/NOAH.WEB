        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          ...
          <div className="flex flex-col justify-center space-y-6">
            <div className="grid grid-cols-2 gap-4">
              {[
                { key: 'birthDate', label: language === 'en' ? 'Birth Date' : 'Fecha de Nacimiento' },
                { key: 'height', label: language === 'en' ? 'Height' : 'Altura' },
                { key: 'weight', label: language === 'en' ? 'Weight' : 'Peso' },
                { key: 'nationality', label: language === 'en' ? 'Nationality' : 'Nacionalidad' },
              ].map((item, index) => (
                <div
                  key={index}
                  className="p-6 bg-white rounded-lg shadow-lg transform hover:-translate-y-1 transition-transform duration-300"
                >
                  <h4 className="font-semibold text-gray-700 mb-2">{item.label}</h4>
                  <p className="text-lg">{playerData.basicInfo[item.key]}</p>
                </div>
              ))}
            </div>
          </div>
